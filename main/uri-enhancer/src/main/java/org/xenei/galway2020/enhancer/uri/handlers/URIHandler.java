...
public class URIHandler extends URLHandlerImpl {
	...
	protected final URIEnhancer enhancer;
	 ...
	 */
	public URIHandler(URLHandlerFactory factory, MediaType mediaType,
			boolean haveContent, URLConnection connection,
			Resource urlResource, Model updates, URIEnhancer enhancer) {
	 ...
	 */
	public URIHandler(URLHandlerFactory factory, MediaType mediaType,
			boolean haveContent, Resource urlResource, Model updates,
	...
	/**
	 * Handle this in standard fashion with the additional operation to check
	 * for rewriting the result. If the urlResource can be rewritten the
	 * rewritten URL is also handled.
	 */
	@Override
	public void handle() {
		...
		if (haveContent) {
			if (connection.getDate() != 0) {
				DateToRDF.addDate(getWritingResource(),
			...
			if (!connection.getURL().toExternalForm().equals(getURIString())) {
				...
				Resource other = getWritingResource().getModel()
						.createResource(connection.getURL().toExternalForm());
				getWritingResource().addProperty(OWL.sameAs, other);
				other.addProperty(OWL.sameAs, getWritingResource());
			}
		}

		enhancer.addToIgnoreList(getWritingResource());
