...
/**
 * A chain comprising a ModelSource, zero or more Enhancers, a ModelSink and an
 * optional ModelSink requeue.
 * 
 */
public abstract class AbstractWorkChain implements Runnable {
	...
	public AbstractWorkChain(ModelSource source, ModelSink sink,
			ModelSink retryQueue) {
		this(null, source, Collections.<Enhancer> emptyList(), sink, retryQueue);
	}

	public AbstractWorkChain(String graphName, ModelSource source,
			ModelSink sink, ModelSink retryQueue) {
		this(graphName, source, Collections.<Enhancer> emptyList(), sink,
				retryQueue);
	}

	public AbstractWorkChain(ModelSource source, List<Enhancer> enhancers,
			ModelSink sink, ModelSink retryQueue) {
		this(null, source, enhancers, sink, retryQueue);
	}

	public AbstractWorkChain(String graphName, ModelSource source,
			List<Enhancer> enhancers, ModelSink sink, ModelSink retryQueue) {
		this.source = source;
		...
		try {
			...
			while (iter.hasNext()) {
				...
				try {
					...
					getLog().warn(
							"Exception thrown during attempted sink: "
									+ e.getMessage(), e);
					// on any exception send to retryQueue
					try {
						performSink(graphName, retryQueue, model);
					} catch (Exception e2) {
						getLog().error(
								"Unable to send model to sink or retryQueue -- DATA LOST",
								e2);
					}
				} finally {
					model.close();
